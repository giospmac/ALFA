...
def render_black_litterman_page() -> None:
    ...
    with st.expander("Configurações Básicas", expanded=False):
        ...
        # Nao pode ser um expander: o Streamlit proibe expander dentro de expander.
        delta_val, tau_val = config.delta, config.tau
        if st.toggle("🔧 Modo avançado (parâmetros do algoritmo)", value=False, key="bl_advanced"):
    ...
    fig.add_trace(go.Bar(
        ...
        name="Equilíbrio", marker_color="#1d3a9c",
    ))
    fig.add_trace(go.Bar(
    ...
    fig.update_layout(
        ...
        yaxis=dict(title="Retorno anualizado", tickformat=".1%", showgrid=True, gridcolor="#e2e0d8"),
    ...
    if not marginal_df.empty:
        ...
        fig2.update_layout(
            ...
            font=dict(color="#5a6178", size=11, family="Inter"),
            margin=dict(l=0, r=20, t=30, b=30),
            yaxis=dict(title="Contribuição ao retorno", tickformat=".2%", showgrid=True, gridcolor="#e2e0d8"),
            xaxis=dict(showgrid=False),
        )
