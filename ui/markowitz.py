...
_CUSTOM_BLUES = [[0.0, "#8aa8fa"], [0.35, "#4979f6"], [0.7, "#1d3a9c"], [1.0, "#101a3a"]]
...
def _build_frontier_chart(
    ...
    if not result.frontier.empty:
        ...
        if not upper.empty:
            fig.add_trace(go.Scatter(
                ...
                mode="lines", line=dict(color="#1d3a9c", width=2.5),
                name="Fronteira eficiente", hoverinfo="skip",
            ))
        if not lower.empty:
            fig.add_trace(go.Scatter(
                x=lower["volatilidade"], y=lower["retorno"],
                mode="lines", line=dict(color="#8aa8fa", width=1.5, dash="dash"),
    ...
    if result.tangency and result.tangency.volatility > 1e-6:
        ...
        fig.add_trace(go.Scatter(
            x=cml_x, y=cml_y,
            mode="lines", line=dict(color="#8aa8fa", width=1.5, dash="dot"),
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers",
        marker=dict(size=14, symbol="star", color="#4979f6", line=dict(color="#1d3a9c", width=1.5)),
        name="Máximo Sharpe",
    ...
    fig.add_trace(go.Scatter(
        ...
        mode="markers",
        marker=dict(size=10, symbol="diamond", color="#1d3a9c", line=dict(color="#101a3a", width=1.5)),
    ...
    if current_vol is not None and current_ret is not None:
        fig.add_trace(go.Scatter(
            ...
            mode="markers",
            marker=dict(size=12, symbol="circle", color="#8aa8fa", line=dict(color="#8aa8fa", width=1.5)),
            name="Carteira atual",
            hovertemplate="Vol: %{x:.2%}<br>Ret: %{y:.2%}<extra></extra>",
        ))
    ...
    fig.update_xaxes(
        title_text="", showgrid=True, gridcolor="#e2e0d8", gridwidth=1,
        zeroline=False, showline=True, linecolor="#e2e0d8", linewidth=1, tickformat=".0%",
        range=x_range,
    )
    fig.update_yaxes(
        title_text="", showgrid=True, gridcolor="#e2e0d8", gridwidth=1,
