...
CHART_COLORS = ["#4979f6", "#1d3a9c", "#101a3a", "#6fa0ff", "#3355cc", "#0d1a4a", "#8ab4ff", "#2a4da6", "#15306b"]
...
def _render_styles() -> None:
    """Sem estilos locais: a aparência vem do design system em `theme/styles.py`."""
...
def _performance_card(title: str, ticker: str, total_return: float) -> str:
    ...
    return (
        f"<div class='alfa-tickercard'>"
        f"<div class='alfa-tickercard__label'>{title}</div>"
        f"<div class='alfa-tickercard__value'>{ticker}</div>"
        f"<div class='alfa-tickercard__pill {direction}'>{arrow} {abs(total_return) * 100:.2f}%</div>"
        f"</div>"
    )
...
def _apply_alfa_style(fig: go.Figure, title: str = "") -> go.Figure:
    fig.update_layout(
        title=dict(
            text=title,
            font=dict(color="#14192e", size=14, family="Inter"),
            pad=dict(b=10)
        ) if title else None,
...
def _plot_return_vs_volatility(metrics_list) -> None:
    ...
    fig.add_trace(go.Bar(
        ...
        marker_color="#101a3a", width=0.35
    ))
...
def _plot_correlation_heatmap(history: pd.DataFrame) -> None:
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        x=tickers,
        y=tickers,
        colorscale=[[0, "#97bdff"], [0.5, "#4979f6"], [1.0, "#101a3a"]],
        zmin=-1, zmax=1,
