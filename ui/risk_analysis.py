...
def render_risk_analysis_page() -> None:
    ...
    if metrics is None:
        ...
        if len(x_scatter) > 1:
            try:
                ...
                fig_scatter.add_trace(go.Scatter(
                    ...
                    mode="lines",
                    name="Tendência",
                    line=dict(color="#8890a5", width=1, dash="dash"),
                    hoverinfo="skip"
                ))
            except Exception:
                pass
        ...
        fig_scatter.update_layout(
            ...
            font=dict(color="#5a6178", size=11, family="Inter"),
            margin=dict(l=0, r=20, t=30, b=40),
            showlegend=False,
            xaxis=dict(title="", tickformat=".0%", showgrid=False, zeroline=False, showline=True, linecolor="#e2e0d8", linewidth=1),
            yaxis=dict(title="", tickformat=".0%", showgrid=True, gridcolor="#e2e0d8", gridwidth=1, zeroline=False, showline=False)
        )
    ...
    if asset_table is not None and not asset_table.empty:
        ...
        fig_risk.add_trace(go.Bar(
            ...
            name="Sistêmico", marker_color="#1d3a9c"
        ))
        fig_risk.add_trace(go.Bar(
        ...
        fig_risk.update_layout(
            ...
            xaxis=dict(showgrid=False, zeroline=False, showline=True, linecolor="#e2e0d8", linewidth=1),
            yaxis=dict(title="", tickformat=".0%", showgrid=True, gridcolor="#e2e0d8", gridwidth=1, zeroline=False, showline=False)
        )
        st.plotly_chart(fig_risk, use_container_width=True, theme=None, config={"displayModeBar": False})
        ...
        fig_sml.add_trace(go.Scatter(
            ...
            line=dict(color="#1d3a9c", width=2, dash="dash"),
        ...
        fig_sml.update_layout(
            ...
            margin=dict(l=0, r=20, t=30, b=40),
            legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
            xaxis=dict(title="Beta de Mercado", showgrid=False, zeroline=False, showline=True, linecolor="#e2e0d8", linewidth=1),
            yaxis=dict(title="Retorno Anualizado", tickformat=".1%", showgrid=True, gridcolor="#e2e0d8", gridwidth=1, zeroline=False, showline=False)
        )
        st.plotly_chart(fig_sml, use_container_width=True, theme=None, config={"displayModeBar": False})
    else:
