...
def _performance_card(title: str, ticker: str, expected_return: float) -> str:
    ...
    return (
        ...
        f"<div class='alfa-tickercard__pill {direction}'>{arrow} {abs(expected_return) * 100:.2f}%</div>"
        f"</div>"
    )
...
def _plot_projection(history: pd.DataFrame, projected: pd.DataFrame) -> None:
    ...
    if not history.empty:
        ...
        fig.add_vline(x=cutoff, line_width=1.5, line_dash="dash", line_color="#8890a5")
